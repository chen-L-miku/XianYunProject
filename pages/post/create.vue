<template xmlns:v-quill='富文本编辑器'>
  <div class="container">
    <el-row :gutter="70">
      ...
      <el-col class="gt" :span="4">
        <div class="grid-content bg-purple drafts">
          {{`草稿箱（${num}）`}}
          <div style="margin: 10px 0" v-for="(item,index) in save" :key="index">
            <!-- <i :class="item.writes"></i> -->
            <el-link :underline="false" class="enter" @click="reStrategy(item)"><span>{{item.title}}<i :class="item.writes"></i></span></el-link>
            <div style="font-size: 12px;color: #999">{{item.saveDate}}</div>
          </div>
        </div>
      </el-col>
    </el-row>
